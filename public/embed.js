 ...
 * the infrahiveChatbotConfig should be defined in the html file before this script is included
 * the infrahiveChatbotConfig should contain the token of the chatbot
...
async function embedChatbot () {
  const infrahiveChatbotConfig = window.infrahiveChatbotConfig;
  if (!infrahiveChatbotConfig || !infrahiveChatbotConfig.token) {
  ...
  function createIframe () {
    ...
    iframe.title = "infrahive chatbot bubble window"
    iframe.id = 'infrahive-chatbot-bubble-window'
    iframe.src = `${baseUrl}/chatbot/${infrahiveChatbotConfig.token}`
  ...
  const targetButton = document.getElementById('infrahive-chatbot-bubble-button')
  if (!targetButton) {
    // create button
    const containerDiv = document.createElement("div");
    containerDiv.id = 'infrahive-chatbot-bubble-button'
    containerDiv.style.cssText = `position: fixed; bottom: 1rem; right: 1rem; width: 50px; height: 50px; border-radius: 25px; background-color: #fed44d; box-shadow: rgba(0, 0, 0, 0.2) 0px 4px 8px 0px; cursor: pointer; z-index: 2147483647; transition: all 0.2s ease-in-out 0s; left: unset; transform: scale(1); :hover {transform: scale(1.1);}`;
    ...
    containerDiv.addEventListener('click', function () {
      const targetIframe = document.getElementById('infrahive-chatbot-bubble-window')
      if (!targetIframe) {
        createIframe()
